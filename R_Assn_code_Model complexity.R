...
m1
#TRAIN AND TEST ACCURACY
tr=predict(m1,newdata=train3)
etr1=sum((tr-(mean(train3$MPG)))^2)
etr1
pred = predict(m1, newdata=test3)
ets1=sum((pred-(mean(test3$MPG)))^2)
ets1
...
m2
#TRAIN AND TEST ACCURACY
tr=predict(m2,newdata=train3)
etr2=sum((tr-(mean(train3$MPG)))^2)
etr2
pred = predict(m2, newdata=test3)
ets2=sum((pred-(mean(test3$MPG)))^2)
ets2
# fit regression model of order3
m3<-lm(MPG~Weight+I(Weight^2)+I(Weight^3),train3)
m3
#TRAIN AND TEST ACCURACY
tr=predict(m3,newdata=train3)
etr3=sum((tr-(mean(train3$MPG)))^2)
etr3
pred = predict(m3, newdata=test3)
ets3=sum((pred-(mean(test3$MPG)))^2)
ets3
...
m5
#TRAIN AND TEST ACCURACY
tr=predict(m5,newdata=train3)
etr5=sum((tr-(mean(train3$MPG)))^2)
etr5
pred = predict(m5, newdata=test3)
ets5=sum((pred-(mean(test3$MPG)))^2)
ets5
...
m7
#TRAIN AND TEST ACCURACY
tr=predict(m7,newdata=train3)
etr7=sum((tr-(mean(train3$MPG)))^2)
etr7
pred = predict(m7, newdata=test3)
ets7=sum((pred-(mean(test3$MPG)))^2)
ets7

# fit regression model of order9
m9<-lm(MPG~Weight+I(Weight^2)+I(Weight^3)+I(Weight^4)+I(Weight^5)+I(Weight^6)+I(Weight^7)+I(Weight^8)+I(Weight^9),train3)
m9
#TRAIN AND TEST ACCURACY in RMSE

tr=predict(m9,newdata=train3)
etr9=sum((tr-(mean(train3$MPG)))^2)
etr9
pred = predict(m9, newdata=test3)
ets9=sum((pred-(mean(test3$MPG)))^2)
ets9
...
#plotting the error vs model complexity for n=100
x=c(1,2,3,5,7,9)
y=c(2200,2350,2500,2650,2800,2950)
...
plot(x,y, main="RMSE vs. Model Complexity for n=50",type="n", 
     xlab="Model complexity", ylab="RMSE", pch=18, col="blue")
lines(x, ytr[order(x)], col='black', type='l',pch=50)
lines(x, yts[order(x)], col='red', type='l',pch=50)
legend(x=6.724648, y=2387.669, legend=c("Train RMSE", "Test RMSE"),
